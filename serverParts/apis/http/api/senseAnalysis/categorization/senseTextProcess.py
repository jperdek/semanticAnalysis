...
class SemcorAnalyser:
    ...
    @staticmethod
    def get_json_domain_vector_results(domain_vector):
        result_array = list()
        for category_name, value in domain_vector.items():
            if value > 0:
                record = dict()
        ...
        result_array = sorted(result_array, key=lambda k: k['value'], reverse=True)
        return result_array
    ...
    def analyse_text_semcor_results(self, text, k, repeat=2, use_normal_dist=False, debug=False,
                                    describe=False, return_results=False):
        dest_text_domain_vector = self.create_vector_for_domain()
        words_text = text.split()
        length_text = len(words_text)
        print(type(length_text))
        ...
        if return_results:
            result_json = dict()
            result_json = self.add_configuration_information(result_json, text, k)
            result_json['results'] = self.get_json_domain_vector_results(dest_text_domain_vector)
            return result_json
        else:
            return self.print_domain_vector_results(dest_text_domain_vector)

    @staticmethod
    def add_configuration_information(result_json, text, k):
        result_json['analyzed_text'] = text
        result_json['window'] = k
        return result_json


if __name__ == "__main__":
    semcorAnalyser = SemcorAnalyser("../../../../../../output/domain-lookup/wordnet/domain-parts.json", "semcor_frequencies.json")
