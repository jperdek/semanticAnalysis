...
if __name__ == '__main__':
    ...
    while True:
        ...
        if file_data_are_same('clusters' + str(iteration - 1) + '.txt', 'clusters' + str(iteration) + '.txt'):
            ...
            break
        if cleaning and iteration > 4:
            print("Cleaning created local text files....")
            cleaned_iteration = iteration - 3
            os.remove("data" + str(cleaned_iteration) + ".txt")
            os.remove("clusters" + str(cleaned_iteration) + ".txt")
            os.remove("data_clusters" + str(cleaned_iteration) + ".txt")
