...
import sys
import os
save_path = "hdfs:///temp/InitProcessingResults/"
...
class KMedoid(MRJob):
    ...
    def get_data_file_path(iteration: int):
        return save_path + '/' + str(iteration) + '/data' + str(iteration) + '.txt'
...
def kmedoid():
    ...
    run_job(KMedoid, [
        base_path,
        '-r=hadoop',
        '--output-dir=' + save_path + '1/', '--iteration=1'])
    iteration = 1
    while True:
        iteration = iteration + 1
        actual_cluster_file_path = save_path + '/' + str(iteration - 1) + '/clusters' + str(iteration - 1) + '.txt'
        print(save_path + '/' + str(iteration - 1) + '/clusters' + str(iteration - 1) + '.txt')
        actual_data_file_path = save_path + '/' + str(iteration - 1) + '/data' + str(iteration - 1) + '.txt'
        print("Starting iteration: " + str(iteration))
        run_job(KMedoid, [
            actual_data_file_path, '-r=hadoop',
            '--output-dir=' + save_path, '--iteration=' + str(iteration)])
        ...
        if remove_old:
            if iteration > 3:
                os.remove(save_path + '/' + str(int(iteration) - 2) + '/clusters' + str(int(iteration) - 2) + '.txt')
                os.remove(save_path + '/' + str(int(iteration) - 2) + '/data' + str(int(iteration) - 2) + '.txt')
